...
reef_fish_data <- read.csv("../reef_fish_data_haw.csv")

################create a family list of endemics and get all the names

family_endlist <- c("Albulidae", "Muraenidae", "Chlopsidae", "Ophichthidae", "Congridae", "Synodontidae",
                    "Ophidiidae" , "Bythitidae", "Antennariidae" , "Isonidae", "Holocentridae", "Syngnathidae",
                    "Scorpaenidae", "Serranidae", "Priacanthidae", "Apogonidae", "Mullidae", "Kyphosidae",
                    "Chaetodontidae", "Pomacanthidae", "Pentacerotidae", "Oplegnathidae", "Cirrhitidae",
                    "Cheilodactylidae", "Mugilidae", "Pomacentridae", "Labridae", "Scaridae", "Tripterygiidae",
                    "Blenniidae", "Callionymidae", "Gobiidae", "Kraemeriidae", "Ephippidae", "Acanthuridae",
                    "Sphyraenidae", "Balistidae", "Monacanthidae", "Ostraciidae", "Tetraodontidae" )
...
for (i in family_endlist) {
  all_famend[[i]] <- rfishbase::species_list(Family = i )
   ...
   #########extract species from OG csv into a
  ###dataframe with only endemic species
